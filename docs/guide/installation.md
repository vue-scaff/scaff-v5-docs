...
浏览器会打开一个示例页。Vue 和 React 模板都使用同一套 Core 资源协议；Vue 页面直接使用 `$store.home` 等自动暴露属性，React 当前需要通过 `useScaff()` 和 Core 查询 API 读取资源。React 尚未实现 Vue 式的零 import 组件消费与 `$type.namespace` 暴露。
